...
public class BookServiceImpl implements BookService {
		...
		try {
			...
			if (read) {
				...
				for (Account account : bookReaders) {
					Point p = accountDao.findEvaluationPoint(bookId, EthraaConstants.ACTIVITY_TYPE_READ_BOOK,
							account.getId());
					Long points = 0l;
					if (p != null) {
						points = p.getPoints();
					...
					BookReaderWrapper item = new BookReaderWrapper(account, points);
					result.add(item);
				}
			} else {
	...
	public void evaluate(EvaluationRequest evaluation) throws EthraaException {
		...
		Point p = accountDao.findEvaluationPoint(evaluation.getBookId(), EthraaConstants.ACTIVITY_TYPE_READ_BOOK,
		...
		if (p != null) {
			...
			point.setType(EthraaConstants.ACTIVITY_TYPE_READ_BOOK);
